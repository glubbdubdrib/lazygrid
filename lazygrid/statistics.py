...
def confidence_interval_mean(x: np.ndarray, cl: float = 0.05) -> tuple([float, float]):
    """
    ...
    >>>
    >>> confidence_interval_mean(x, confidence_level)
...
def cross_validation(model: Union[Sequential, ABCMeta, Pipeline],
    ...
    for split_index in range(0, n_splits):
        ...
        # transform predictions into integer labels
...
def find_best_solution(solutions: list,
    ...
    >>>
    >>> score1, _ = cross_validation(model=model1, x=x, y=y, db_name="database", dataset_id=1, dataset_name="make-class")
    >>> score2, _ = cross_validation(model=model2, x=x, y=y, db_name="database", dataset_id=1, dataset_name="make-class")
    >>> score3, _ = cross_validation(model=model3, x=x, y=y, db_name="database", dataset_id=1, dataset_name="make-class")
    >>>
    >>> scores = [score1["train_cv"], score2["train_cv"], score3["train_cv"]]
    ...
    [6.5464...e-05, 0.4812..., 6.4670...e-05]
...
def compare_models(models: list,
                   ...
                   random_data: bool = True, random_model: bool = True,
                   seed: int = 42, n_splits: int = 10, metric: str = "f1",
                   test: Callable = mannwhitneyu, alpha: int = 0.05, cl: int = 0.05,
    ...
    >>> results[['db-name', 'model', 'mean', 'ci-l-bound', 'ci-u-bound', 'separable', 'pvalue']]
          db-name model      mean  ci-l-bound  ci-u-bound separable    pvalue
    0  make-class    10  0.959798    0.909641    1.009955     False  1.000000
    1  make-class    20  0.928662    0.851488    1.005835     False  0.532541
    2  make-class    30  0.948864    0.896696    1.001031     False  0.654039
